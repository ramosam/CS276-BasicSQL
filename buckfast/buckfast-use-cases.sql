...
START WITH 10014 -- current store_inventory_lineitem
INCREMENT BY 1;
...
VALUES (STORE_INVENTORY_LINEITEM_ID_ARG, PROD_ID_ARG, SHIPMENT_ID_ARG, (SELECT SUM((
SELECT SILI.QuanTITY
FROM BASTORE_INVENTORY_LINEITEM SILI
JOIN BAPRODUCT P ON P.PRODUCT_ID = SILI.FK_PRODUCT_ID
WHERE FK_PRODUCT_ID = 'SKU1001') + 12)
FROM DUAL));
COMMIT;
...
-- BEFORE UPDATE
SELECT * FROM BASTORE_INVENTORY_LINEITEM WHERE FK_PRODUCT_ID = 'SKU1001';


-- RUNNING PROCEDURE

BEGIN
  UPDATE_PRODUCT_INVENTORY_COUNT(STORE_INVENTORY_LINEITEM_SEQ.NEXTVAL, 'SKU1001', 'SHIP-018', 12.50, 12);
END;
/


-- AFTER UPDATE
SELECT P.PRODUCT_ID, P.PRODUCT_PRICE, SILI.QUANTITY 
FROM BASTORE_INVENTORY_LINEITEM SILI
JOIN BAPRODUCT P ON P.PRODUCT_ID =  SILI.FK_PRODUCT_ID
