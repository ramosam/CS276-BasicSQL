...
SELECT SUM(EXTRACT(HOUR FROM BASHIFT.END_DATE_TIME) - EXTRACT(HOUR FROM BASHIFT.START_DATE_TIME)) 
*
(SELECT DISTINCT PAY_RATE
  FROM BAPAYCHECK
  WHERE BAPAYCHECK.FK_EMPLOYEE_ID = 'L107') AS PAY_AMOUNT
...
/*
  insert ('SHIP-00'||store_inventory_lineitem_seq.nextval).....
*/
...
/
BEGIN
  UPDATE_PRODUCT_INVENTORY_COUNT(STORE_INVENTORY_LINEITEM_SEQ.NEXTVAL, 'SKU1001', 'SHIP-016', 12);
END;
/

-- FIND THE VENDOR, FIND THE STORE THAT WOULD ORDER THIS
SELECT V.VENDOR_NAME 
FROM BAVENDOR V
JOIN BASHIPMENT SHIP ON SHIP.FK_VENDOR_ID = V.VENDOR_ID
...
WHERE P.PRODUCT_ID = 'SKU1001';

SELECT SI.FK_BUSINESS_ID, SI.STORE_INVENTORY_ID
FROM BAPRODUCT P 
JOIN BASTORE_INVENTORY_LINEITEM SILI ON P.PRODUCT_ID = SILI.FK_PRODUCT_ID
JOIN BASHIPMENT SHIP ON SHIP.SHIPMENT_ID = SILI.FK_SHIPMENT_ID
JOIN BASTORE_INVENTORY SI ON SI.STORE_INVENTORY_ID = SHIP.FK_STORE_INVENTORY_ID
...
--lpad(next, nubmer total of values before key, '0' - for the padded value to be used)
--  insert into mytable values ('bat'||lpad(seq.nextval, 3, '0'));
